		   ...
		   <tbody>
		   	<c:forEach items="${list_cart}" var="cart">
			     <tr>
			       ...
			       <!-- 复选框表示是否被选中 -->
			       <td><input type="checkbox" onclick="change_shfxz(this.checked,${cart.sku_id})" ${cart.shfxz=="1"?"checked":""}/>${cart.shfxz}<img src="upload/image/${cart.shp_tp}" alt="" width="80px"></td>
			       <td>${cart.sku_mch}</td>
			       <td>
			       ...
			       <td ><a href="javascript:delete_cart(${cart.id },${cart.yh_id })">删除</a></td>
			     </tr>
			</c:forEach>
		   </tbody>
	 	</table>
